...
const contentOptions = removedStorage => ({
  allowedTags: ['p', 'div', 'br', 'strong', 'b', 'em', 'i', 'u', 's', 'font', 'h2', 'h3', 'ul', 'ol', 'li', 'blockquote', 'a', 'img', 'figure', 'figcaption', 'pre', 'code'],
  allowedAttributes: { a: ['href', 'target', 'rel'], div: ['style'], p: ['style'], h2: ['style'], h3: ['style'], blockquote: ['style'], li: ['style'], font: ['color', 'size'], img: ['src', 'alt', 'title', 'data-attachment-id', 'style'] },
  allowedStyles: { '*': { 'text-align': [/^(?:left|center|right)$/], 'line-height': [/^(?:1\.25|1\.6|1\.85|2\.2)$/] }, img: { width: [/^(?:25|50|75|100)%$/], height: [/^auto$/] } },
...
async function init() {
  await query(`CREATE TABLE IF NOT EXISTS themes (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
    ...
    CREATE TABLE IF NOT EXISTS articles (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, title_color TEXT NOT NULL DEFAULT '#253229', summary TEXT NOT NULL DEFAULT '', content TEXT NOT NULL DEFAULT '', content_format TEXT NOT NULL DEFAULT 'html', written_date TEXT, language TEXT NOT NULL DEFAULT 'pt-BR', theme_id INTEGER REFERENCES themes(id), created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
...
async function saveArticle(id, payload) {
  ...
  const client=await pool.connect(); try { await client.query('BEGIN'); let articleId=id;
    if(id){const r=await client.query("UPDATE articles SET title=$1,title_color=$2,summary=$3,content=$4,content_format='html',written_date=$5,language=$6,theme_id=$7,updated_at=now() WHERE id=$8",[fields.title,fields.title_color,fields.summary,fields.content,fields.written_date,fields.language,fields.theme_id,id]);if(!r.rowCount)throw new Error('Artigo não encontrado.');}
    else articleId=(await client.query("INSERT INTO articles(title,title_color,summary,content,content_format,written_date,language,theme_id) VALUES($1,$2,$3,$4,'html',$5,$6,$7) RETURNING id",[fields.title,fields.title_color,fields.summary,fields.content,fields.written_date,fields.language,fields.theme_id])).rows[0].id;
